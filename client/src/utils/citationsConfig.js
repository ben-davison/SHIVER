// web/client/src/utils/citationsConfig.js
...
export function generateCitationText(usedSources, currentRegion) {
  const dateDownloaded = new Date().toISOString().split('T')[0];
  
  // Ensure the region string is lowercase to match the object keys
  const regionKey = currentRegion.toLowerCase(); 
  const regionCitations = citationsConfig[regionKey] || {};
  
  let text = "=========================================\n";
  text += " SHIVER DATA EXTRACTION CITATION RECORD\n";
  text += "=========================================\n\n";
  text += `Date Extracted: ${dateDownloaded}\n`;
  text += `Region: ${currentRegion.toUpperCase()}\n\n`;
  ...
  citationsConfig.common["SHIVER"].forEach(cit => {
      text += `* ${cit}\n`;
  });
  text += "\n";
  ...
  usedSources.forEach(source => {
    if (regionCitations[source]) {
      const citations = Array.isArray(regionCitations[source]) 
        ? regionCitations[source] 
        : [regionCitations[source]];
      ...
      text += `* WARNING: Missing citation for source '${source}' in region '${currentRegion}'\n\n`;
    }
  });
